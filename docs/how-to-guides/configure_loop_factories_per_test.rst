...
``pytest_asyncio_loop_factories`` is called with the pytest ``config`` and
current test ``item``. Use the item to decide which named event loop factories
are available for the test being collected.
...
In ``conftest.py``, check ``item.fixturenames`` and build the factory mapping
for that test item:
...
The same pattern works with information available from the current pytest item,
such as fixture names, markers, node IDs, or file paths.
...
For declaring factories without item-specific logic, see
