...
==================================
Determines the default event loop scope of asynchronous fixtures. When this configuration option is unset, it defaults to ``function``. Possible values are: ``function``, ``class``, ``module``, ``package``, ``session``.
